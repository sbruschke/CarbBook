...
describe('pullChanges', () => {
  it('returns seeded dose settings from since=0 with JSON decoded', () => {
    ...
    expect(page.changes.map((c) => c.table)).toEqual(['dose_settings', 'dose_settings']);
  ...
  it('pages across tables in server_seq order', () => {
    ...
    const first = pullChanges(db, 2, 2);
    expect(first.changes.map((c) => [c.table, c.record.id, c.record.server_seq])).toEqual([
      ['meal', 'm1', 3],
      ['food', 'f1', 4],
    ]);
    expect(first).toMatchObject({ next_since: 4, has_more: true });
    const second = pullChanges(db, first.next_since, 2);
    expect(second.changes.map((c) => c.record.id)).toEqual(['m2']);
    expect(second).toMatchObject({ next_since: 5, has_more: false });
    expect(pullChanges(db, 5, 2)).toEqual({ changes: [], next_since: 5, has_more: false });
  });
  ...
  it('includes soft-deleted records so clients learn about deletes', () => {
    ...
    const page = pullChanges(db, 2, 10);
    expect(page.changes).toHaveLength(1);
    expect(page.changes[0]!.record).toMatchObject({ id: 'f1', deleted: 1, server_seq: 4 });
  });
